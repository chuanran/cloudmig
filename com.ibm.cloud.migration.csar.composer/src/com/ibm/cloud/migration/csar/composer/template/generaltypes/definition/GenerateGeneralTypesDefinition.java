...
public class GenerateGeneralTypesDefinition {
	...
	static{
		...
		velocity_property.setProperty("file.resource.loader.path", "./tosca_templates/generaltypes_defiition_template/");
	...
	public static void generateGeneralTypesDefinitionFile() throws Exception{
		...
		String app_types_definition = "";
			...
			//velocity template for "GeneralTypes_Diagram/Runtime.template"
			 ...
			 String runtime_diagram = "";
			 for(WebAppNode webapp : web_apps){
				 context_runtime_diagram.put(TOSCATemplateConstants.RUNTIME_KEY, app_container);
				 context_runtime_diagram.put(TOSCATemplateConstants.APP_NAME_KEY, webapp.getName());
				 ...
				 runtime_diagram = runtime_diagram + "\n" + writer_runtime_diagram.toString(); 
			 }
			 if(runtime_diagram.trim().length() >0){
				 context_general_types_diagram.put(TOSCATemplateConstants.RUNTIME_NODE_DIAGRAM_KEY, runtime_diagram);
			 } else{
				 context_general_types_diagram.put(TOSCATemplateConstants.RUNTIME_NODE_DIAGRAM_KEY, "");
			 }
...
//			System.out.println("service_instance_types_definition is: " + service_instance_types_definition);
			...
			for(WebAppNode webapp : webapps){
				...
				String hostedOnRuntime ="";
				try {
					hostedOnRuntime = webapp.getHostedOnContainer().getBuildPack().toString();
				} catch (AppModelException e) {
					// TODO 自动生成 catch 块
					e.printStackTrace();
				}
