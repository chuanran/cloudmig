...
//			}
	...
	private void genContainerAndRelation(AppModel model, CloudFoundryClient cfClient, List<CloudApplication> apps){
		for(CloudApplication app : apps){
			...
			String staging_info = cfClient.getFile(app.getName(), 0, "staging_info.yml");
			//System.out.println("staging info is: " + staging_info);
//			String buildpack = app.getStaging().getBuildpackUrl();

			//FIXME: to support more buildpacks
			//here only supports "node", if the staging info contains "node", it tips the buildpack is nodejs 
			if(staging_info.toLowerCase().contains("node")){
				//System.out.println("unsupported build pack ");
...
//			}	
	...
	public AppModel buildAppModel(CloudFoundryClient cfClient, List<CloudApplication> apps, 
	...
	private void downloadAppFiles(CloudFoundryClient cfClient, List<CloudApplication>  apps){
		...
		if(answer.equals("yes")||answer.equals("y")){
			for(CloudApplication app : apps){
				//FIXME: download app.
				// looks like cloud foundry has no way to know whether a path is directory or file.				
			}
		}
	}
	...
	public void export(String targetUrl, String username, String password, String outfile) 
		...
		if(selApps.size()>1) {
			...
			String dependency_answer = dependency_answer_reader.nextLine();
...
//			String outfile = reader.nextLine();
			...
			CFExporter exporter = new CFExporter();
