if(process.env.VCAP_SERVICES){
	...
	for(var i in service_arr){
		...
		if(service.url){
			process.env.MONGOLAB_URI = service.url;
		}else{
			if(service.username && service.password){		
				 process.env.MONGOLAB_URI = "mongodb://" + service.username + ":" + service.password + "@"
					   + service.hostname + ":" + service.port + "/" + service.db;
		    }else{
				 process.env.MONGOLAB_URI = "mongodb://" + service.hostname + ":" + service.port + "/" + service.db;
			}
		}
	}
}
